...
// 求a[n][n]的逆参数有很大限制，必须是a[n][n]格式的参数 ，所有探求在形式上改变，以增强适应性					
{
/* a is matrix n X n
n:is row and line number
return value :
1 :ok;
	0 :fail */
...
// 2. int nCols - 指定的矩阵列数
//////////////////////////////////////////////////////////////////////
ArxRleMatrix::ArxRleMatrix(int nRows, int nCols)
{
	m_nNumRows = nRows;
	m_nNumColumns = nCols;
	m_pData = NULL;
...
//////////////////////////////////////////////////////////////////////
ArxRleMatrix::ArxRleMatrix(int nRows, int nCols, double value[])
{
	m_nNumRows = nRows;
	m_nNumColumns = nCols;
	m_pData = NULL;
...
// 1. int nSize - 方阵行列数
//////////////////////////////////////////////////////////////////////
ArxRleMatrix::ArxRleMatrix(int nSize)
{
	m_nNumRows = nSize;
	m_nNumColumns = nSize;
	m_pData = NULL;
...
//////////////////////////////////////////////////////////////////////
ArxRleMatrix::ArxRleMatrix(int nSize, double value[])
{
	m_nNumRows = nSize;
	m_nNumColumns = nSize;
	m_pData = NULL;
...
//
// 参数：
// 1. const ArxRleMatrix& other - 源矩阵
//////////////////////////////////////////////////////////////////////
ArxRleMatrix::ArxRleMatrix(const ArxRleMatrix& other)
{
...
// 返回值：BOOL 型，初始化是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::Init(int nRows, int nCols)
{
	if (m_pData)
	{
		delete[] m_pData;
...
// 返回值：BOOL 型，初始化是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::MakeUnitMatrix(int nSize)
{
	if (! Init(nSize, nSize))
		return FALSE;
...
// 返回值：BOOL 型，转换是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::FromString(CString s, const CString& sDelim /*= " "*/, BOOL bLineBreak /*= TRUE*/)
{
	if (s.IsEmpty())
		return FALSE;
	...
	if (bLineBreak)
	{
		...
		ArxRleTokenizer tkRow(sRow, sDelim);
		CString sElement;
		// 列数
		m_nNumColumns = 0;
	...
	sElement.TrimLeft();
	sElement.TrimRight();
	m_nNumRows = _tstoi(sElement);

	// 列数
	tk.Next(sElement);
	sElement.TrimLeft();
	sElement.TrimRight();
	m_nNumColumns = _tstoi(sElement);
...
//////////////////////////////////////////////////////////////////////
CString ArxRleMatrix::ToString(int nWidth,int precision,const CString& sDelim /*= " "*/, BOOL bLineBreak /*= TRUE*/) const
{
	CString s=L"";
	...
	for (int i=0; i<m_nNumRows; ++i)
	{
		for (int j=0; j<m_nNumColumns; ++j)
		{
			CString ss;
	    	CString rule;
		    rule.Format(L"%s%d%s%df",L"%",nWidth,L".",precision);
			ss.Format(rule, GetElement(i, j));
			s += ss;
...
// 返回值：CString 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
CString ArxRleMatrix::RowToString(int nRow, const CString& sDelim /*= " "*/) const
{
	CString s = L"";
	...
	{
		CString ss;
		ss.Format(L"%f", GetElement(nRow, j));
		s += ss;
		if (j != m_nNumColumns-1)
			s += sDelim;
	}
...
// 返回值：CString 型，转换得到的字符串
//////////////////////////////////////////////////////////////////////
CString ArxRleMatrix::ColToString(int nCol, const CString& sDelim /*= " "*/) const
{
	CString s = L"";
	...
	{
		CString ss;
		ss.Format(L"%f", GetElement(i, nCol));
		s += ss;
		if (i != m_nNumRows-1)
			s += sDelim;
	}
...
// 返回值：无
//////////////////////////////////////////////////////////////////////
void ArxRleMatrix::SetData(double value[])
{
	// empty the memory
...
void ArxRleMatrix::SetData(double x)
{
	if(m_pData==NULL)return;
	//memset(m_pData, x, sizeof(double) * m_nNumColumns*m_nNumRows);
	for(int j=0;j<this->GetNumRows();j++)
	{
		for(int i=0;i<this->GetNumColumns();i++)
		{
			m_pData[j*GetNumRows()+i]=x;
		}
	}
}
//////////////////////////////////////////////////////////////////////
...
// 返回值：无
//////////////////////////////////////////////////////////////////////
void ArxRleMatrix::SetRow(int nRow,double *pData)
{
	for(int j=0;j<m_nNumColumns;j++)
	SetElement(	nRow,j,pData[j]);
}

void ArxRleMatrix::SetCol(int nCol,double *pData)
{
	for(int i=0;i<m_nNumColumns;i++)
	SetElement(	i,nCol,pData[i]);
}
//////////////////////////////////////////////////////////////////////
// 设置指定元素的值
//
...
// 返回值：BOOL 型，说明设置是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::SetElement(int nRow, int nCol, double value)
{
...
// 返回值：BOOL
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::SaveToFile(CString szPath)
{
	FILE*f;
	_wfopen_s(&f,szPath,L"a");
	CString sData;
	sData=this->ToString();
	fprintf(f,"%s",sData);
	fclose(f);
...
// 输出到文件
//
// 参数：CString szPath, int nWidth, int precision
//
// 返回值：BOOL 型
//////////////////////////////////////////////////////////////////////

BOOL ArxRleMatrix::SaveToFile(CString szPathname, int nWidth, int precision,const CString& sDelim, BOOL bLineBreak)
{
	FILE*f;
	_wfopen_s(&f,szPathname,L"w");
	if(!f)return FALSE;
	fprintf(f,"%s",ToString(nWidth,precision,sDelim,bLineBreak));
	fclose(f);		
	return TRUE;
}
...
// 返回值：double 型，指定元素的值
//////////////////////////////////////////////////////////////////////
double ArxRleMatrix::GetElement(int nRow, int nCol) const
{
...
// 返回值：int 型，矩阵的列数
//////////////////////////////////////////////////////////////////////
int	ArxRleMatrix::GetNumColumns() const
{
...
// 返回值：int 型，矩阵的行数
//////////////////////////////////////////////////////////////////////
int	ArxRleMatrix::GetNumRows() const
{
	return m_nNumRows;
}
...
//////////////////////////////////////////////////////////////////////
double* ArxRleMatrix::GetData() const
{
	return m_pData;
}
...
//////////////////////////////////////////////////////////////////////
int ArxRleMatrix::GetRowVector(int nRow, double* pVector) const
{
	if (pVector == NULL)
		delete pVector;
...
//////////////////////////////////////////////////////////////////////
int ArxRleMatrix::GetColVector(int nCol, double* pVector) const
{
	if (pVector == NULL)
		delete pVector;
...
// 3. int nHight- 子矩阵行数
// 4. int nWidth- 子矩阵列数
// 5.ArxRleMatrix& other 返回的子矩阵
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::GetSubMatrix(ArxRleMatrix& other,int i,int j,int nHight,int nWidth)
{
	if(nHight>m_nNumRows||nWidth>m_nNumColumns)return 	FALSE;
	if(i+nHight>m_nNumRows||nWidth+j>m_nNumColumns)return FALSE;
	other.Init(nHight,nWidth);
	double data;
	int row,col;
	...
	for(int k=0;k<nHight;k++)
	{
		for(int l=0;l<nWidth;l++)
		{
			row=i+k;
			col=j+l;
			data=this->GetElement(row,col);
			other.SetElement(k,l,data);
		}
	}
...
// 返回值：int 型，向量中元素的个数，即矩阵的行数
//////////////////////////////////////////////////////////////////////
ArxRleMatrix ArxRleMatrix::GetSubMatrix(int i,int j,int nHight,int nWidth)
{
	ArxRleMatrix other;
	if(nHight>m_nNumRows||nWidth>m_nNumColumns)return other;
	if(i+nHight>m_nNumRows||nWidth+j>m_nNumColumns)return other;
	other.Init(nHight,nWidth);
	double data;
	int row,col;
	for(int k=0;k<nHight;k++)
...
//
// 参数：
// 1. const ArxRleMatrix& other - 用于给矩阵赋值的源矩阵
//
// 返回值：ArxRleMatrix型的引用，所引用的矩阵与other相等
//////////////////////////////////////////////////////////////////////
ArxRleMatrix& ArxRleMatrix::operator=(const ArxRleMatrix& other)
{
	if(other.GetData()==NULL)return *this;
	if (&other != this)
	{
...
// 返回值：ArxRleMatrix型的引用，所引用的矩阵与other相等
//////////////////////////////////////////////////////////////////////
ArxRleMatrix& ArxRleMatrix::operator=(double* pData)
{
	SetData(pData);
	return *this;
}
...
//
// 参数：
// 1. const ArxRleMatrix& other - 用于比较的矩阵
//
// 返回值：BOOL 型，两个矩阵相等则为TRUE，否则为FALSE
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::operator==(const ArxRleMatrix& other) const
{
	// 首先检查行列数是否相等
...
//
// 参数：
// 1. const ArxRleMatrix& other - 与指定矩阵相加的矩阵
//
// 返回值：ArxRleMatrix型，指定矩阵与other相加之和
//////////////////////////////////////////////////////////////////////
ArxRleMatrix	ArxRleMatrix::operator+(const ArxRleMatrix& other) const
{
	// 首先检查行列数是否相等
	ASSERT (m_nNumColumns == other.GetNumColumns() && m_nNumRows == other.GetNumRows());

	// 构造结果矩阵
	ArxRleMatrix	result(*this) ;		// 拷贝构造
	// 矩阵加法
	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
		for (int j = 0 ; j <  m_nNumColumns; ++j)
...
//
// 参数：
// 1. const ArxRleMatrix& other - 与指定矩阵相减的矩阵
//
// 返回值：ArxRleMatrix型，指定矩阵与other相减之差
//////////////////////////////////////////////////////////////////////
ArxRleMatrix	ArxRleMatrix::operator-(const ArxRleMatrix& other) const
{
	// 首先检查行列数是否相等
	...
	// 构造目标矩阵
	ArxRleMatrix	result(*this) ;		// copy ourselves
	// 进行减法操作
	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
		for (int j = 0 ; j <  m_nNumColumns; ++j)
...
//
// 返回值：ArxRleMatrix型，指定矩阵与value相乘之积
//////////////////////////////////////////////////////////////////////
ArxRleMatrix	ArxRleMatrix::operator*(double value) const
{
	// 构造目标矩阵
	ArxRleMatrix	result(*this) ;		// copy ourselves
	// 进行数乘
...
//
// 参数：
// 1. const ArxRleMatrix& other - 与指定矩阵相乘的矩阵
//
// 返回值：ArxRleMatrix型，指定矩阵与other相乘之积
//////////////////////////////////////////////////////////////////////
ArxRleMatrix	ArxRleMatrix::operator*(const ArxRleMatrix& other) const
{
	// 首先检查行列数是否符合要求
	...
	ArxRleMatrix	result(m_nNumRows, other.GetNumColumns()) ;
...
// 返回值：double
//////////////////////////////////////////////////////////////////////
double ArxRleMatrix::operator()(int i,int j)const
{
	double err;
	err=1.0e100;
	if(i>=m_nNumRows||i<0||j>=m_nNumColumns||j<0)return err;

	return m_pData[i*m_nNumColumns+j];
}
//返回第i行指针;
double*  ArxRleMatrix::operator[](int i)const
{
	return (m_pData+i*m_nNumColumns);
}
//////////////////////////////////////////////////////////////////////
// 复矩阵的乘法
//
// 参数：
// 1. const ArxRleMatrix& AR - 左边复矩阵的实部矩阵
// 2. const ArxRleMatrix& AI - 左边复矩阵的虚部矩阵
// 3. const ArxRleMatrix& BR - 右边复矩阵的实部矩阵
// 4. const ArxRleMatrix& BI - 右边复矩阵的虚部矩阵
// 5. ArxRleMatrix& CR - 乘积复矩阵的实部矩阵
// 6. ArxRleMatrix& CI - 乘积复矩阵的虚部矩阵
//
// 返回值：BOOL型，复矩阵乘法是否成功
//////////////////////////////////////////////////////////////////////

BOOL ArxRleMatrix::CMul(const ArxRleMatrix& AR, const ArxRleMatrix& AI, const ArxRleMatrix& BR, const ArxRleMatrix& BI, ArxRleMatrix& CR, ArxRleMatrix& CI) const
{
	// 首先检查行列数是否符合要求
	...
	// 构造乘积矩阵实部矩阵和虚部矩阵
	ArxRleMatrix mtxCR(AR.GetNumRows(), BR.GetNumColumns()), mtxCI(AR.GetNumRows(), BR.GetNumColumns());
	// 复矩阵相乘
...
// 参数：无
//
// 返回值：ArxRleMatrix型，指定矩阵转置矩阵
//////////////////////////////////////////////////////////////////////
ArxRleMatrix ArxRleMatrix::T() const
{
	// 构造目标矩阵
	ArxRleMatrix	Trans(m_nNumColumns, m_nNumRows);
...
// 返回值：BOOL型，求逆是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::InvertGaussJordan()
{
...
//
// 参数：
// 1. ArxRleMatrix& mtxImag - 复矩阵的虚部矩阵，当前矩阵为复矩阵的实部
//
// 返回值：BOOL型，求逆是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::InvertGaussJordan(ArxRleMatrix& mtxImag)
{
...
// 返回值：BOOL
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::inv()
{
	if(m_nNumColumns!=m_nNumRows)return false;
	BOOL ret=false;
	ret=dcinv(m_pData,m_nNumColumns);
	return ret;
}
//////////////////////////////////////////////////////////////////////
// 矩阵的求逆 
...
// 返回值：ArxRleMatrix&
//////////////////////////////////////////////////////////////////////
ArxRleMatrix& ArxRleMatrix::Getinv()
{
	if(m_nNumColumns!=m_nNumRows)return *this;
	ArxRleMatrix *m=new ArxRleMatrix;
	*m=*this;
	dcinv(m->GetData(),m_nNumColumns);
	return *m;
}
//////////////////////////////////////////////////////////////////////
// 托伯利兹矩阵求逆的埃兰特方法
...
// 返回值：BOOL型，求逆是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::InvertTrench()
{ 
	int i,j,k;
...
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::DetCholesky(double* dblDet)
{ 
	int i,j,k,u,l;
    double d;
...
//
// 参数：
// 1. ArxRleMatrix& mtxL - 返回分解后的L矩阵
// 2. ArxRleMatrix& mtxU - 返回分解后的U矩阵
//
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::SplitLU(ArxRleMatrix& mtxL, ArxRleMatrix& mtxU)
{ 
	int i,j,k,w,v,ll;
...
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::LU(ArxRleMatrix& mtxL, ArxRleMatrix& mtxU)
{
	if(m_nNumColumns!=m_nNumRows)return false;
	BOOL ret=false;
	mtxL.Init(m_nNumRows,m_nNumColumns);
	mtxU.Init(m_nNumRows,m_nNumColumns);
	ret=djlu(m_pData,m_nNumColumns,mtxL.m_pData,mtxU.m_pData);
	return ret;
}
//////////////////////////////////////////////////////////////////////
...
//
// 参数：
// 1. ArxRleMatrix& mtxQ - 返回分解后的Q矩阵
//
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::SplitQR(ArxRleMatrix& mtxQ)
{ 
	int i,j,k,l,nn,p,jj;
    double u,alpha,w,t;
...
//
// 参数：
// 1. ArxRleMatrix& mtxU - 返回分解后的U矩阵
// 2. ArxRleMatrix& mtxV - 返回分解后的V矩阵
// 3. double eps - 计算精度，默认值为0.000001
//
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::SplitUV(ArxRleMatrix& mtxU, ArxRleMatrix& mtxV, double eps /*= 0.000001*/)
{ 
...
// 内部函数，由SplitUV函数调用
//////////////////////////////////////////////////////////////////////
void ArxRleMatrix::ppp(double a[], double e[], double s[], double v[], int m, int n)
{ 
	int i,j,p,q;
    double d;
...
// 内部函数，由SplitUV函数调用
//////////////////////////////////////////////////////////////////////
void ArxRleMatrix::sss(double fg[2], double cs[2])
{ 
	double r,d;
...
//
// 参数：
// 1. ArxRleMatrix& mtxAP - 返回原矩阵的广义逆矩阵
// 2. ArxRleMatrix& mtxU - 返回分解后的U矩阵
// 3. ArxRleMatrix& mtxV - 返回分解后的V矩阵
// 4. double eps - 计算精度，默认值为0.000001
//
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::GInvertUV(ArxRleMatrix& mtxAP, ArxRleMatrix& mtxU, ArxRleMatrix& mtxV, double eps /*= 0.000001*/)
{ 
	int i,j,k,l,t,p,q,f;
...
//
// 参数：
// 1. ArxRleMatrix& mtxQ - 返回豪斯荷尔德变换的乘积矩阵Q
// 2. ArxRleMatrix& mtxT - 返回求得的对称三对角阵
// 3. double dblB[] - 一维数组，长度为矩阵的阶数，返回对称三对角阵的主对角线元素
// 4. double dblC[] - 一维数组，长度为矩阵的阶数，前n-1个元素返回对称三对角阵的次对角线元素
//
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::MakeSymTri(ArxRleMatrix& mtxQ, ArxRleMatrix& mtxT, double dblB[], double dblC[])
{ 
	int i,j,k,u;
    double h,f,g,h2;
...
// 3. ArxRleMatrix& mtxQ - 如果传入单位矩阵，则返回实对称三对角阵的特征值向量矩阵；
...
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::SymTriEigenv(double dblB[], double dblC[], ArxRleMatrix& mtxQ, int nMaxIt /*= 60*/, double eps /*= 0.000001*/)
{
	int i,j,k,m,it,u,v;
...
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::HBergEigenv(double dblU[], double dblV[], int nMaxIt /*= 60*/, double eps /*= 0.000001*/)
{ 
...
// 2. ArxRleMatrix& mtxEigenVector - 返回时存放特征向量矩阵，其中第i列为与
...
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::JacobiEigenv(double dblEigenValue[], ArxRleMatrix& mtxEigenVector, int nMaxIt /*= 60*/, double eps /*= 0.000001*/)
{ 
...
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::JacobiEigenv2(double dblEigenValue[], ArxRleMatrix& mtxEigenVector, double eps /*= 0.000001*/)
{ 
	int i,j,p,q,u,w,t,s;
...
// 1.precision        作为输出精度
// 2.width		      输出宽度
// 3.flag			  ios::scientific or ios::fixed
//
//////////////////////////////////////////////////////////////////////////
void ArxRleMatrix::Show(int width,int precision ,int flag/*ios::scientific*/)
{
	for(int i=0;i<m_nNumRows;i++)
	{
		for(int j=0;j<m_nNumColumns;j++)
...
// 从dat文件中读出数据,并构造矩阵;
//////////////////////////////////////////////////////////////////////////
BOOL ArxRleMatrix::LoadFromDatFile(CString szPath,const CString& sDelim /*= " "*/, BOOL bLineBreak /*= TRUE*/)
{
	if(szPath == L"")return FALSE;
	CFile file;
	if(!file.Open(szPath,CFile::modeRead))return FALSE;
	CString sdata;
	DWORD datasize;
	datasize=(DWORD)file.GetLength();

	file.Read(sdata.GetBuffer(datasize),datasize);
	this->FromString(sdata.GetBuffer(datasize),sDelim,bLineBreak);
	return TRUE;
}
