...
IMPLEMENT_DYNAMIC( ArxRleCoolBar, CToolBar )
BEGIN_MESSAGE_MAP( ArxRleCoolBar, CToolBar )
	//{{AFX_MSG_MAP( ArxRleCoolBar )
	ON_WM_NCCALCSIZE()
	ON_WM_CREATE()
	ON_WM_NCPAINT()
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::SetIconMode( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
UINT ArxRleCoolBar::GetIconMode() const
{
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::SetTextMode( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
bool ArxRleCoolBar::GetTextMode() const
{
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
BOOL ArxRleCoolBar::LoadToolBar( 
     UINT _ResourceId )     // @Parm The toolbar resource id.
{
    ...
    lpszResourceName.Format( L"#%d", _ResourceId );
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::SetImageList( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::AssignImageList()
{
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::InitImageList()
{
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
CSize ArxRleCoolBar::CalcButtonSize()
{
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::SetHotImageList( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::SetStandardImageList( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::SetDisableImageList( 
     CImageList* pList )    // @Parm The normal image list.
{
    // Retrieve the tool bar control and set the list reference.
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::SetButtonDropDown( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::ResizeToolBar()
{
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::DrawGripper( 
...
//            replace the CCmdUI by ArxRleCoolCmdUI.
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::OnUpdateCmdUI( 
     CFrameWnd* pTarget,        // @Parm ??
     BOOL bDisableIfNoHndler )  // @Parm ??
{
	ArxRleCoolCmdUI state; // this is the only line that's different--PD
	state.m_pOther = this;
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::OnNcCalcSize(
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::OnSysColorChange() 
{
    // Call the base class method.
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//            displayed.
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBar::OnWindowPosChanging( LPWINDOWPOS _pWindowPos ) 
{
    // Default processing
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolCmdUI::Enable( BOOL bOn )
{
...
//            Came from Paul Dilascia Article.
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolCmdUI::SetCheck( int nCheck )
{
...
CSize ArxRleCoolBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	if ((nLength == -1)	&& 
...
//
CSize ArxRleCoolBar::GetButtonSize(TBBUTTON* pData, int iButton)
{
...
//
void ArxRleCoolBar::GetButton(int nIndex, TBBUTTON* pButton) const
{
...
//
void ArxRleCoolBar::SetButton(int nIndex, TBBUTTON* pButton)
{
...
CSize ArxRleCoolBar::CalcSize(TBBUTTON* pData, int nCount)
{
...
int ArxRleCoolBar::WrapToolBar(TBBUTTON* pData, int nCount, int nWidth)
{
	ASSERT(pData != NULL && nCount > 0);
...
void  ArxRleCoolBar::SizeToolBar(TBBUTTON* pData, int nCount, int nLength, BOOL bVert)
{
	ASSERT(pData != NULL && nCount > 0);
...
CSize ArxRleCoolBar::CalcLayout(DWORD dwMode, int nLength)
{
	ASSERT_VALID(this);
...
CSize ArxRleCoolBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
...
// Macro and other definition.
// ------------------------------------------------------------------------
IMPLEMENT_DYNAMIC( ArxRleCoolBmpBar, ArxRleCoolBar )
BEGIN_MESSAGE_MAP( ArxRleCoolBmpBar, ArxRleCoolBar )
	//{{AFX_MSG_MAP( ArxRleCoolBmpBar )
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
//            This is the main constructor.
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
ArxRleCoolBmpBar::ArxRleCoolBmpBar()
{
}
...
//            To set the image list in the control.
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBmpBar::InitImageList()
{
    // Build the image list.
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    for ( int i=0; i<NB_POSSIBLE_MODE; i++ )
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBmpBar::BuildImageList( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolBmpBar::SetBitmap( 
...
// Macro and other definition.
// ------------------------------------------------------------------------
IMPLEMENT_DYNAMIC( ArxRleCoolIconBar, ArxRleCoolBar )
BEGIN_MESSAGE_MAP( ArxRleCoolIconBar, ArxRleCoolBar )
	//{{AFX_MSG_MAP( ArxRleCoolIconBar )
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
//            This is the main constructor.
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
ArxRleCoolIconBar::ArxRleCoolIconBar()
{
}
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
HICON ArxRleCoolIconBar::ConvertIconToGrayScale( 
...
// @end =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void ArxRleCoolIconBar::SetIcon( 
