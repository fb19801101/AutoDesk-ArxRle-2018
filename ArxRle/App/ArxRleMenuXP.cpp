//*************************************************************************
// ArxRleMenuXP.cpp : implementation file
// Version : 3.034
// Date : May 2002
// Author : Brent Corkum
...
#include "ArxRleMenuXP.h"        // ArxRleMenuXP class declaration
...
UINT ArxRleMenuXP::original_drawmode=CMenuXP_DRAWMODE_ORIGINAL;
BOOL ArxRleMenuXP::original_select_disabled=TRUE;
// how the menu's are drawn in winXP
UINT ArxRleMenuXP::xp_drawmode=CMenuXP_DRAWMODE_XP;
BOOL ArxRleMenuXP::xp_select_disabled=FALSE;
BOOL ArxRleMenuXP::xp_draw_3D_bitmaps=TRUE;
BOOL ArxRleMenuXP::hicolor_bitmaps=FALSE;
...
BOOL ArxRleMenuXP::xp_space_accelerators=TRUE;
BOOL ArxRleMenuXP::original_space_accelerators=TRUE;
COLORREF ArxRleMenuXP::m_clrMenuXPBk=GetSysColor(COLOR_WINDOW);
COLORREF ArxRleMenuXP::m_clrMenuXPSideBarBk=GetSysColor(COLOR_3DFACE);
COLORREF ArxRleMenuXP::m_clrMenuXPText=GetSysColor(COLOR_MENUTEXT);
COLORREF ArxRleMenuXP::m_clrMenuXPSel=GetSysColor(COLOR_HIGHLIGHT);


CImageList ArxRleMenuXP::m_AllImages;
CArray<int,int&> ArxRleMenuXP::m_AllImagesID;
int ArxRleMenuXP::m_iconX = 16;
int ArxRleMenuXP::m_iconY = 16;
...
void ArxRleMenuXPData::SetAnsiString(LPCSTR szAnsiString)
{
	USES_CONVERSION;
	SetWideString(A2W(szAnsiString));  //SK:  see MFC Tech Note 059
}

CString ArxRleMenuXPData::GetString(void)//returns the menu text in ANSI or UNICODE
...
CTypedPtrArray<CPtrArray, HMENU> ArxRleMenuXP::m_AllSubMenus;  // Stores list of all sub-menus

IMPLEMENT_DYNAMIC( ArxRleMenuXP, CMenu )

/*
===============================================================================
ArxRleMenuXP::ArxRleMenuXP()
ArxRleMenuXP::~ArxRleMenuXP()
-----------------
...
ArxRleMenuXP::ArxRleMenuXP()
{
	...
	//m_clrMenuXPBk=GetSysColor(COLOR_WINDOW);
	//m_clrMenuXPSideBarBk=GetSysColor(COLOR_3DFACE);
...
BOOL ArxRleMenuXP::IsNewShell ()
{
	return (g_Shell>=Win95);
}

BOOL ArxRleMenuXP::IsWinXPLuna()
{
	if(g_Shell==WinXP)
	{
...
BOOL ArxRleMenuXP::IsLunaMenuStyle()
{
	if(IsWinXPLuna()){
		if(xp_drawmode==CMenuXP_DRAWMODE_XP)return(TRUE);
	}
	else{
		if(original_drawmode==CMenuXP_DRAWMODE_XP)return(TRUE);
	}
	return(FALSE);
}

ArxRleMenuXPData::~ArxRleMenuXPData()
{
	if(bitmap)
		delete(bitmap);
...
void ArxRleMenuXPData::SetWideString(const wchar_t *szWideString)
{
	delete[] m_szMenuText;//Need not check for NULL because ANSI X3J16 allows "delete NULL"
...
BOOL ArxRleMenuXP::IsMenu(CMenu *submenu)
{
	int m;
...
BOOL ArxRleMenuXP::IsMenu(HMENU submenu)
{
	int m;
	int numSubMenus = m_AllSubMenus.GetUpperBound();
	for(m=0;m<=numSubMenus;++m){
...
BOOL ArxRleMenuXP::DestroyMenu()
{
	...
	for(m = numSubMenus; m >= 0; m--){
		...
		if(ptr){
			BOOL flag=ptr->IsKindOf(RUNTIME_CLASS( ArxRleMenuXP ));
			if(flag)delete((ArxRleMenuXP *)ptr);
		}
	}
...
int ArxRleMenuXP::GetMenuDrawMode(void)
{
	if(IsWinXPLuna())return(xp_drawmode);
	return(original_drawmode);
}

BOOL ArxRleMenuXP::GetSelectDisableMode(void)
{
...
/*
==========================================================================
void ArxRleMenuXP::DrawItem(LPDRAWITEMSTRUCT)
---------------------------------------
  ...
  ==========================================================================
*/
void ArxRleMenuXP::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	...
	{
	    //若当前系统是WindowsXP系统，并且启用了XP桌面主题，IsWinXPLuna()=FALSE
		if(IsWinXPLuna())
		{
			//绘制Office XP风格菜单
			if(xp_drawmode==CMenuXP_DRAWMODE_XP)
		...
		{
			//绘制Windows 2000风格菜单
			if(original_drawmode==CMenuXP_DRAWMODE_XP)
				DrawItem_WinXP(lpDIS);
			else 
				DrawItem_Win9xNT2000(lpDIS);
		}	
...
void ArxRleMenuXP::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
	CRect rect;
	UINT state = (((ArxRleMenuXPData*)(lpDIS->itemData))->nFlags);
	...
	if(state & MF_SEPARATOR){
		...
		if(lpDIS->itemData != NULL){
			nIconNormal = (((ArxRleMenuXPData*)(lpDIS->itemData))->menuIconNormal);
			xoffset = (((ArxRleMenuXPData*)(lpDIS->itemData))->xoffset);
			global_offset = (((ArxRleMenuXPData*)(lpDIS->itemData))->global_offset);
			bitmap = (((ArxRleMenuXPData*)(lpDIS->itemData))->bitmap);
			strText = ((ArxRleMenuXPData*) (lpDIS->itemData))->GetString();
		...
		if(state&ODS_SELECTED){ // draw the down edges
			...
			if(checkflag||standardflag||selectedflag||disableflag||state&ODS_CHECKED)
				rect2.SetRect(rect.left+m_iconX+4+ArxRleMenuXP_GAP,rect.top,rect.right,rect.bottom);
		...
		rect.left = rect.left + m_iconX + 8 + ArxRleMenuXP_GAP; 
...
COLORREF ArxRleMenuXP::LightenColor(COLORREF col,double factor)
{
...
COLORREF ArxRleMenuXP::DarkenColor(COLORREF col,double factor)
{
	if(factor>0.0&&factor<=1.0){
		BYTE red,green,blue,lightred,lightgreen,lightblue;
		red = GetRValue(col);
...
void ArxRleMenuXP::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	ASSERT(lpDIS != NULL);
	CDC* pDC = CDC::FromHandle(lpDIS->hDC);
#ifdef ArxRleMenuXP_USE_MEMDC
	ArxRleMemDC *pMemDC=NULL;
#endif
	//在后面的绘制函数中，如FillRect()，DrawEdge()函数
	//rect代表菜单项整个用户绘制区域，rect2代表菜单左边图标栏区域
	CRect rect,rect2;
	//得到该菜单项状态
	UINT state = (((ArxRleMenuXPData*)(lpDIS->itemData))->nFlags);
	//m_clrMenuXPBk,菜单背景色
	COLORREF m_clrBack=m_clrMenuXPBk;//GetSysColor(COLOR_WINDOW);
    //m_clrMenuXPSideBarBk,菜单左边图标框背景色
	COLORREF m_newclrBack=m_clrMenuXPSideBarBk;//GetSysColor(COLOR_3DFACE);
	...
	//菜单中图标栏宽度
	int barwidth=m_iconX+CMenuXP_PAD;
	
	// 如果itemState中ODS_GRAYED和ODS_SELECTED位都被设置，则清除ODS_SELECTED位
	if(lpDIS->itemState & ODS_GRAYED&&!xp_select_disabled)
	{
		if(lpDIS->itemState & ODS_SELECTED)
		   lpDIS->itemState=lpDIS->itemState & ~ODS_SELECTED;
#ifdef ArxRleMenuXP_USE_MEMDC
		pMemDC=new ArxRleMemDC(pDC,&lpDIS->rcItem);
		pDC = pMemDC;
	...
	if(state & MF_SEPARATOR)
	{
		...
		rect.top+=rect.Height()>>1;
		rect.left = rect2.right+CMenuXP_PAD;
		pDC->DrawEdge(&rect,EDGE_ETCHED,BF_TOP);
	...
	{   //下面是绘制菜单项
		...
		//standardflag,普通状态；selectedflag被选中状态；disableflag，禁止状态
		BOOL standardflag=FALSE,selectedflag=FALSE,disableflag=FALSE;
		//checkflag,复选框状态
		BOOL checkflag=FALSE;

		//菜单中文本颜色
		COLORREF crText = m_clrMenuXPText;//GetSysColor(COLOR_MENUTEXT);
		//菜单中被选中项的透明框的边框色
		COLORREF crSelect = m_clrMenuXPSel;//GetSysColor(COLOR_HIGHLIGHT);
        //菜单中被选中项的透明框的填充色
		...
		// 绘制弹起/按下/获得焦点/禁止这些状态下的菜单项	
		UINT state = lpDIS->itemState;
		CString strText;
		
		if(lpDIS->itemData != NULL)
		{
			//菜单项对应图标（ico），若nIconNormal=-1
			//则菜单采用bitmap中的bmp图像作为菜单左边图标栏中图标
			nIconNormal = (((ArxRleMenuXPData*)(lpDIS->itemData))->menuIconNormal);
			//绘制菜单项图标，实现3D效果时所用的x方向位移
			xoffset = (((ArxRleMenuXPData*)(lpDIS->itemData))->xoffset);
			//菜单对应的图像列表对象（bmp）
			bitmap = (((ArxRleMenuXPData*)(lpDIS->itemData))->bitmap);
			//菜单项文本
			strText = ((ArxRleMenuXPData*) (lpDIS->itemData))->GetString();
			//绘制菜单项图标，实现3D效果时所用的一个位置偏移量
			global_offset = (((ArxRleMenuXPData*)(lpDIS->itemData))->global_offset);
...
#ifdef ArxRleMenuXP_USE_MEMDC
	if(pFont)
	   pDC->SelectObject (pFont); //恢复设备环境的原有字体
	m_fontMenu.DeleteObject();
	if(pMemDC)delete pMemDC;
#endif
}

BOOL ArxRleMenuXP::GetBitmapFromImageList(CDC* pDC,CImageList *imglist,int nIndex,CBitmap &bmp)
{
...
/*
==========================================================================
void ArxRleMenuXP::MeasureItem(LPMEASUREITEMSTRUCT)
---------------------------------------------
...
void ArxRleMenuXP::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	UINT state = (((ArxRleMenuXPData*)(lpMIS->itemData))->nFlags);
	int CMenuXP_PAD=4;
	if(IsLunaMenuStyle()&&xp_draw_3D_bitmaps)CMenuXP_PAD=7;
	if(state & MF_SEPARATOR){
		lpMIS->itemWidth = 0;
		...
		if(IsLunaMenuStyle())
			...
			lpMIS->itemHeight = temp>(m_iconY+CMenuXP_PAD)/2 ? temp : (m_iconY+CMenuXP_PAD)/2;
	}
	else{
		CFont m_fontMenu;
		LOGFONT m_lf;
		...
		const wchar_t *lpstrText = ((ArxRleMenuXPData*)(lpMIS->itemData))->GetWideString();//SK: we use const to prevent misuse
		...
		if(IsLunaMenuStyle())lpMIS->itemWidth = m_iconX+CMenuXP_PAD+8+t.cx;
		else lpMIS->itemWidth = m_iconX + t.cx + m_iconX + ArxRleMenuXP_GAP;
		int temp = GetSystemMetrics(SM_CYMENU);
		lpMIS->itemHeight = temp>m_iconY+CMenuXP_PAD ? temp : m_iconY+CMenuXP_PAD;
		m_fontMenu.DeleteObject();
	}
}

void ArxRleMenuXP::SetIconSize (int width, int height)
{
	m_iconX = width;
	m_iconY = height;
}

BOOL ArxRleMenuXP::AppendODMenuA(LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL ArxRleMenuXP::AppendODMenuW(wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	...
	ArxRleMenuXPData *mdata = new ArxRleMenuXPData;
...
BOOL ArxRleMenuXP::InsertODMenuA(UINT nPosition,LPCSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL ArxRleMenuXP::InsertODMenuW(UINT nPosition,wchar_t *lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	if(!(nFlags & MF_BYPOSITION)){
		int iPosition =0;
		ArxRleMenuXP* pMenu = FindMenuOption(nPosition,iPosition);
		if(pMenu){
...
BOOL ArxRleMenuXP::ModifyODMenuA(const char * lpstrText,UINT nID,int nIconNormal)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,nIconNormal);//SK: see MFC Tech Note 059
}

BOOL ArxRleMenuXP::ModifyODMenuW(wchar_t *lpstrText,UINT nID,int nIconNormal)
{
	int nLoc;
	ArxRleMenuXPData *mdata;
	CArray<ArxRleMenuXP*,ArxRleMenuXP*>bcsubs;
	CArray<int,int&>bclocs;
	
	// Find the old ArxRleMenuXPData structure:
	ArxRleMenuXP *psubmenu = FindMenuOption(nID,nLoc);
	do{
		if(psubmenu && nLoc>=0)mdata = psubmenu->m_MenuList[nLoc];
		else{
			// Create a new ArxRleMenuXPData structure:
			mdata = new ArxRleMenuXPData;
			m_MenuList.Add(mdata);
		}
...
BOOL ArxRleMenuXP::ModifyODMenuA(const char * lpstrText,UINT nID,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,il,xoffset);
}

BOOL ArxRleMenuXP::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CImageList *il,int xoffset)
{
	int nLoc;
	ArxRleMenuXPData *mdata;
	CArray<ArxRleMenuXP*,ArxRleMenuXP*>bcsubs;
...
BOOL ArxRleMenuXP::ModifyODMenuA(const char * lpstrText,UINT nID,CBitmap *bmp)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,bmp);
}

BOOL ArxRleMenuXP::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
...
BOOL ArxRleMenuXP::ModifyODMenuA(const char * lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,fill,border,hatchstyle,pSize);
}

BOOL ArxRleMenuXP::ModifyODMenuW(wchar_t *lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
	CWnd *pWnd = AfxGetMainWnd();            // Get main window
	CDC *pDC = pWnd->GetDC();              // Get device context
	SIZE sz;
...
BOOL ArxRleMenuXP::ModifyODMenuA(const char *lpstrText,const char *OptionText,
                           int nIconNormal)
{
	USES_CONVERSION;
...
BOOL ArxRleMenuXP::ModifyODMenuW(wchar_t *lpstrText,wchar_t *OptionText,
                           int nIconNormal)
{
	ArxRleMenuXPData *mdata;
...
ArxRleMenuXPData *ArxRleMenuXP::NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string)
{
	ArxRleMenuXPData *mdata;
...
BOOL ArxRleMenuXP::LoadToolbars(const UINT *arID,int n)
{
	ASSERT(arID);
...
BOOL ArxRleMenuXP::LoadToolbar(UINT nToolBar)
{
	UINT nID,nStyle;
...
BOOL ArxRleMenuXP::LoadFromToolBar(UINT nID,UINT nToolBar,int& xoffset)
{
	int xset,offset;
	UINT nStyle;
	BOOL returnflag=FALSE;
...
ArxRleMenuXPData *ArxRleMenuXP::FindMenuItem(UINT nID)
{
	...
	if (!pData){
		int loc;
		ArxRleMenuXP *pMenu = FindMenuOption(nID, loc);
		ASSERT(pMenu != this);
		if (loc >= 0){
...
ArxRleMenuXP *ArxRleMenuXP::FindAnotherMenuOption(int nId,int& nLoc,CArray<ArxRleMenuXP*,ArxRleMenuXP*>&bcsubs,
									  CArray<int,int&>&bclocs)
{
	int i,numsubs,j;
	ArxRleMenuXP *psubmenu,*pgoodmenu;
	BOOL foundflag;
	
	for(i=0;i<(int)(GetMenuItemCount());++i){
#ifdef _CPPRTTI 
		psubmenu=dynamic_cast<ArxRleMenuXP *>(GetSubMenu(i));
#else
		psubmenu=(ArxRleMenuXP *)GetSubMenu(i);
#endif
		if(psubmenu){
...
ArxRleMenuXP *ArxRleMenuXP::FindMenuOption(int nId,int& nLoc)
{
	int i;
	ArxRleMenuXP *psubmenu,*pgoodmenu;
...
ArxRleMenuXPData *ArxRleMenuXP::FindMenuOption(wchar_t *lpstrText)
{
	int i,j;
	ArxRleMenuXP *psubmenu;
	ArxRleMenuXPData *pmenulist;
...
BOOL ArxRleMenuXP::LoadMenu(int nResource)
{
	return(ArxRleMenuXP::LoadMenu(MAKEINTRESOURCE(nResource)));
};

BOOL ArxRleMenuXP::LoadMenu(LPCTSTR lpszResourceName)
{
	ASSERT_VALID(this);
	...
	CTypedPtrArray<CPtrArray, ArxRleMenuXP*>  m_Stack;    // Popup menu stack
	...
	do{
		...
		if(dwFlags & MF_POPUP){
			...
			ArxRleMenuXP* pSubMenu = new ArxRleMenuXP;
...
int ArxRleMenuXP::GetMenuStart(void)
{
...
void ArxRleMenuXP::RemoveTopLevelOwnerDraw(void)
{
	CString str;
	int i=0,j=0;
...
void ArxRleMenuXP::InsertSpaces(void)
{
	if(IsLunaMenuStyle())
...
void ArxRleMenuXP::LoadCheckmarkBitmap(int unselect, int select)
{
...
//--------------------------------------------------------------------------
//[18.06.99 rj]
BOOL ArxRleMenuXP::GetMenuText(UINT id, CString& string, UINT nFlags/*= MF_BYPOSITION*/)
{
	BOOL returnflag=FALSE;
	...
	if(MF_BYPOSITION&nFlags){
		...
		int uiLoc;
		ArxRleMenuXP* pMenu = FindMenuOption(id,uiLoc);
...
void ArxRleMenuXP::DrawRadioDot(CDC *pDC,int x,int y,COLORREF color)
{
...
void ArxRleMenuXP::DrawCheckMark(CDC* pDC,int x,int y,COLORREF color,BOOL narrowflag)
{
	int dp=0;
	CPen m_penBack;
...
void ArxRleMenuXP::SynchronizeMenu(void)
{
	CTypedPtrArray<CPtrArray, ArxRleMenuXPData*> temp;
	ArxRleMenuXPData *mdata;
	CString string;
...
void ArxRleMenuXP::UpdateMenu(CMenu *pmenu)
{
#ifdef _CPPRTTI 
	ArxRleMenuXP *psubmenu = dynamic_cast<ArxRleMenuXP *>(pmenu);
#else
	ArxRleMenuXP *psubmenu = (ArxRleMenuXP *)pmenu;
#endif
	if(psubmenu)psubmenu->SynchronizeMenu();
}

LRESULT ArxRleMenuXP::FindKeyboardShortcut(UINT nChar, UINT nFlags,
                                     CMenu *pMenu)
{
#ifdef _CPPRTTI 
	ArxRleMenuXP *pCMenuXP = dynamic_cast<ArxRleMenuXP *>(pMenu);
#else
	ArxRleMenuXP *pCMenuXP = (ArxRleMenuXP *)pMenu;
#endif
	if(pCMenuXP && nFlags&MF_POPUP){
		CString key(_T('&'),2);//SK: modified for Unicode correctness
		key.SetAt(1,(TCHAR)nChar);
		key.MakeLower();
		CString menutext;
		int menusize = (int)pCMenuXP->GetMenuItemCount();
		if(menusize!=(pCMenuXP->m_MenuList.GetUpperBound()+1))
			pCMenuXP->SynchronizeMenu();
		for(int i=0;i<menusize;++i){
			if(pCMenuXP->GetMenuText(i,menutext)){
				menutext.MakeLower();
...
void ArxRleMenuXP::DitherBlt (HDC hdcDest, int nXDest, int nYDest, int nWidth, 
...
void ArxRleMenuXP::GetFadedBitmap(CBitmap &bmp)
{
	CDC ddc;
	COLORREF bgcol,col;
	BITMAP BitMap;
...
void ArxRleMenuXP::GetTransparentBitmap(CBitmap &bmp)
{
	CDC ddc;
...
void ArxRleMenuXP::GetDisabledBitmap(CBitmap &bmp,COLORREF background)
{
	CDC ddc;
...
void ArxRleMenuXP::GetShadowBitmap(CBitmap &bmp)
{
	CDC ddc;
...
BOOL ArxRleMenuXP::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	BOOL bReturn=FALSE;
...
void ArxRleMenuXP::SetBitmapBackground(COLORREF color)
{
	m_bitmapBackground=color;
	m_bitmapBackgroundFlag=TRUE;
}

void ArxRleMenuXP::UnSetBitmapBackground(void)
{
	m_bitmapBackgroundFlag=FALSE;
}
...
void ArxRleMenuXP::AddFromToolBar(CToolBar* pToolBar, int nResourceID)
{
...
BOOL ArxRleMenuXP::Draw3DCheckmark(CDC *dc, const CRect& rc,
...
BOOL ArxRleMenuXP::DrawXPCheckmark(CDC *dc, const CRect& rc, HBITMAP hbmCheck,COLORREF &colorout,BOOL selected)
{
	CBrush brushin;
	COLORREF col;
	int dx,dy;
...
void ArxRleMenuXP::DitherBlt2(CDC *drawdc, int nXDest, int nYDest, int nWidth, 
...
void ArxRleMenuXP::DitherBlt3(CDC *drawdc, int nXDest, int nYDest, int nWidth, 
...
void ArxRleMenuXP::SetDisableOldStyle(void)
{
	disable_old_style=TRUE;
}

void ArxRleMenuXP::UnSetDisableOldStyle(void)
{
	disable_old_style=FALSE;
}

BOOL ArxRleMenuXP::GetDisableOldStyle(void)
{
	return(disable_old_style);
}


WORD ArxRleMenuXP::NumBitmapColors(LPBITMAPINFOHEADER lpBitmap)
{
...
HBITMAP ArxRleMenuXP::LoadSysColorBitmap(int nResourceId)
{
	HINSTANCE hInst = 
...
BOOL ArxRleMenuXP::RemoveMenu(UINT uiId,UINT nFlags)
{
	if(MF_BYPOSITION&nFlags){
		...
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
			...
			ArxRleMenuXP* pSubMenu = (ArxRleMenuXP*) GetSubMenu(uiId);
			if(NULL==pSubMenu){
		...
		int iPosition =0;
		ArxRleMenuXP* pMenu = FindMenuOption(uiId,iPosition);
...
BOOL ArxRleMenuXP::DeleteMenu(UINT uiId,UINT nFlags)
{
	if(MF_BYPOSITION&nFlags){
		UINT uint = GetMenuState(uiId,MF_BYPOSITION);
		if(uint&MF_SEPARATOR && !(uint&MF_POPUP)){
...
BOOL ArxRleMenuXP::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL ArxRleMenuXP::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL ArxRleMenuXP::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL ArxRleMenuXP::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return AppendODMenuW(lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL ArxRleMenuXP::AppendMenuA(UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return AppendMenuW(nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL ArxRleMenuXP::AppendMenuW(UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
BOOL ArxRleMenuXP::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,int nIconNormal)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),nIconNormal);
}

BOOL ArxRleMenuXP::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,int nIconNormal)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,nIconNormal);
}

BOOL ArxRleMenuXP::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CImageList *il,int xoffset)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),il,xoffset);
}

BOOL ArxRleMenuXP::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CImageList *il,int xoffset)
{
	return InsertODMenuW(nPosition,lpszNewItem,nFlags,nIDNewItem,il,xoffset);
}

BOOL ArxRleMenuXP::InsertMenuA(UINT nPosition,UINT nFlags,UINT nIDNewItem,const char *lpszNewItem,CBitmap *bmp)
{
	USES_CONVERSION;
	return InsertMenuW(nPosition,nFlags,nIDNewItem,A2W(lpszNewItem),bmp);
}

BOOL ArxRleMenuXP::InsertMenuW(UINT nPosition,UINT nFlags,UINT nIDNewItem,wchar_t *lpszNewItem,CBitmap *bmp)
{
	if(bmp){
		CImageList temp;
		temp.Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
ArxRleMenuXP* ArxRleMenuXP::AppendODPopupMenuW(wchar_t *lpstrText)
{
	ArxRleMenuXP* pSubMenu = new ArxRleMenuXP;
	pSubMenu->m_unselectcheck=m_unselectcheck;
	pSubMenu->m_selectcheck=m_selectcheck;
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
ArxRleMenuXP* ArxRleMenuXP::AppendODPopupMenuA(LPCSTR lpstrText)
{
	USES_CONVERSION;
	return AppendODPopupMenuW(A2W(lpstrText));
}

BOOL ArxRleMenuXP::ImageListDuplicate(CImageList *il,int xoffset,CImageList *newlist)
{
...
CMenu* ArxRleMenuXP::GetSubMenu(int nPos)
{
	return CMenu::GetSubMenu (nPos);
}

CMenu* ArxRleMenuXP::GetSubMenu(LPCTSTR lpszSubMenuName)
{
...
ArxRleMenuXP* ArxRleMenuXP::GetSubCMenuXP(char* pText)
{
	USES_CONVERSION;
	return GetSubCMenuXP(A2W(pText));
}

ArxRleMenuXP* ArxRleMenuXP::GetSubCMenuXP(wchar_t* lpszSubMenuName)
{
	ArxRleMenuXPData *mdata;
	...
	if(mdata){
		...
		if(ptr){
			BOOL flag=ptr->IsKindOf(RUNTIME_CLASS( ArxRleMenuXP ));
			if(flag)return((ArxRleMenuXP *)ptr);
		}
	}
	return NULL;
}
...
int ArxRleMenuXP::GetMenuPosition(char* pText)
{
	USES_CONVERSION;
	return GetMenuPosition(A2W(pText));
}

int ArxRleMenuXP::GetMenuPosition(wchar_t* pText)
{
	int i,j;
	ArxRleMenuXP* psubmenu;
	for(i=0;i<(int)(GetMenuItemCount());++i)
...
int ArxRleMenuXP::RemoveMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return RemoveMenu(A2W(pText), sPos);
}

int ArxRleMenuXP::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
	...
	if(sPos == CMenuXP_HEAD)
	{
		ASSERT(nPos - 1 >= 0);
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CMenuXP_TAIL)
	{
		RemoveMenu(nPos-1, MF_BYPOSITION);
	}
	else if(sPos == CMenuXP_BOTH)
	{
...
int ArxRleMenuXP::DeleteMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return DeleteMenu(A2W(pText), sPos);
}

int ArxRleMenuXP::DeleteMenu(wchar_t* pText, BC_Seperator sPos)
{
	int nPos = GetMenuPosition(pText);
	if(nPos != -1)
	{
...
// Larry Antram
BOOL ArxRleMenuXP::SetMenuText(UINT id, CString string, UINT nFlags/*= MF_BYPOSITION*/ )
{
	BOOL returnflag=FALSE;
...
// courtesy of Warren Stevens
void ArxRleMenuXP::ColorBitmap(CDC* pDC,CBitmap& bmp,CSize bitmap_size,CSize icon_size,COLORREF fill,COLORREF border,int hatchstyle)
{
	CDC bmpdc;
	COLORREF m_newclrBack;
...
BOOL ArxRleMenuXP::IsWindowsClassicTheme(void)
{
...
int ArxRleMenuXP::GlobalImageListOffset(int nID)
{
...
BOOL ArxRleMenuXP::CanDraw3DImageList(int offset)
{
	BOOL retflag=FALSE;
	int numcurrent=m_AllImagesID.GetSize();
...
int ArxRleMenuXP::AddToGlobalImageList(CImageList *il,int xoffset,int nID)
{
	int loc = -1;
...
void ArxRleMenuXP::SetMenuXPItemColor(COLORREF clrMenuXPBk,COLORREF clrMenuXPSideBarBk,COLORREF clrMenuXPText,COLORREF clrMenuXPSel)
{
    m_clrMenuXPBk=clrMenuXPBk;
	m_clrMenuXPSideBarBk=clrMenuXPSideBarBk;
	m_clrMenuXPText=clrMenuXPText;
	m_clrMenuXPSel=clrMenuXPSel;
}
