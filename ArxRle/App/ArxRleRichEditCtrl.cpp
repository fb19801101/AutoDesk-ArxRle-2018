...
#include "ArxRleRichEditCtrl.h"

/////////////////////////////////////////////////////////////////////////////
// ArxRleRichEditCtrl
ArxRleRichEditCtrl::ArxRleRichEditCtrl()
{
}
...
BEGIN_MESSAGE_MAP(ArxRleRichEditCtrl, CRichEditCtrl)
	//{{AFX_MSG_MAP(ArxRleRichEditCtrl)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

// Character and Font Attributes
int ArxRleRichEditCtrl::IsBold(void)
{ CHARFORMAT cf;	
...
int ArxRleRichEditCtrl::IsItalic(void)
{ CHARFORMAT cf;	
  cf.cbSize = sizeof(CHARFORMAT);
...
int ArxRleRichEditCtrl::IsUnderlined(void)
{ CHARFORMAT cf;	
  cf.cbSize = sizeof(CHARFORMAT);
...
int ArxRleRichEditCtrl::IsStrikeout(void)
{ CHARFORMAT cf;	
  cf.cbSize = sizeof(CHARFORMAT);
...
int ArxRleRichEditCtrl::IsProtected(void)
{ CHARFORMAT cf;	
  cf.cbSize = sizeof(CHARFORMAT);
...
int ArxRleRichEditCtrl::IsLinked(void)
{ CHARFORMAT cf;	
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_LINK;
...
void ArxRleRichEditCtrl::SetBold(void)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_BOLD;
...
void ArxRleRichEditCtrl::SetItalic(void)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_ITALIC;
...
void ArxRleRichEditCtrl::SetUnderlined(void)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_UNDERLINE;
...
void ArxRleRichEditCtrl::SetStrikeout(void)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_STRIKEOUT;
...
void ArxRleRichEditCtrl::SetProtected(void)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_PROTECTED;
...
void ArxRleRichEditCtrl::SetLink(void)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_LINK;
...
int ArxRleRichEditCtrl::GetFontSize(void)
{ CHARFORMAT cf;
  int nPointSize = 0;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_SIZE;
...
void ArxRleRichEditCtrl::SetFontSize(int nPointSize)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_SIZE;
...
CString ArxRleRichEditCtrl::GetFontName(void)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_FACE;
  DWORD dwSelMask = GetSelectionCharFormat(cf);
...
void ArxRleRichEditCtrl::SetFontName(const CString strFontName)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_FACE;
  GetSelectionCharFormat(cf);
...
void ArxRleRichEditCtrl::SetColour(COLORREF color)
{ CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = CFM_COLOR;
...
int ArxRleRichEditCtrl::IsRight()
{ PARAFORMAT pf;
...
int ArxRleRichEditCtrl::IsLeft(void)
{ PARAFORMAT pf;
  pf.cbSize = sizeof(PARAFORMAT);
...
int ArxRleRichEditCtrl::IsCentered(void)
{ PARAFORMAT pf;
  pf.cbSize = sizeof(PARAFORMAT);
  pf.dwMask = PFM_ALIGNMENT;    
...
int ArxRleRichEditCtrl::IsJustified(void)
{ PARAFORMAT pf;
  pf.cbSize = sizeof(PARAFORMAT);
  pf.dwMask = PFM_ALIGNMENT;    
...
int ArxRleRichEditCtrl::IsBulleted(void)
{ PARAFORMAT pf;
  pf.cbSize = sizeof(PARAFORMAT);
...
void ArxRleRichEditCtrl::SetRight(void)
{ PARAFORMAT pf;    
  pf.cbSize     = sizeof(PARAFORMAT);
  pf.dwMask     = PFM_ALIGNMENT;    
...
void ArxRleRichEditCtrl::SetLeft(void)
{ PARAFORMAT pf;    
  pf.cbSize     = sizeof(PARAFORMAT);
  pf.dwMask     = PFM_ALIGNMENT;    
...
void ArxRleRichEditCtrl::SetCenter(void)
{ PARAFORMAT pf;    
  pf.cbSize     = sizeof(PARAFORMAT);
  pf.dwMask     = PFM_ALIGNMENT;    
...
void ArxRleRichEditCtrl::SetJustify(void)
{ PARAFORMAT pf;    
  pf.cbSize     = sizeof(PARAFORMAT);
  pf.dwMask     = PFM_ALIGNMENT;    
...
void ArxRleRichEditCtrl::SetBullet(void)
...
// Visual Appearance
void ArxRleRichEditCtrl::SetWordWrap(const bool bOn/*= true*/,const int iLineWidth/* = 0*/)
{ if( bOn )
...
// Reading and Writing
void ArxRleRichEditCtrl::SetRTF(const CString strText)
...
void ArxRleRichEditCtrl::SetRTF(const CByteArray& arrRTF)
...
void ArxRleRichEditCtrl::SetRTF(const UINT resID)
...
void ArxRleRichEditCtrl::LoadRTF(const CString strFilename)
...
void ArxRleRichEditCtrl::GetRTF(CString& strText,const bool bAsRTF/*=true*/)
{ SCookieString CookieRTF;
  strText.Empty();
  CookieRTF.lSize    = 0;
  CookieRTF.lStart   = 0;
...
void ArxRleRichEditCtrl::GetRTF(CByteArray& arrText, const bool bAsRTF/*=true*/)
{ SCookieByteArray CookieRTF;
  arrText.RemoveAll();
  CookieRTF.lSize    = 0;
  CookieRTF.lStart   = 0;
...
void ArxRleRichEditCtrl::WriteRTF(const CString strFilename, const bool bAsRTF/*=true*/ )
{ SCookieFile CookieRTF;
  CFileStatus fileStatus;
  CString strRTF = _T("");
  CookieRTF.lStart    = 0;
...
DWORD CALLBACK ArxRleRichEditCtrl::StreamInCString(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
...
DWORD CALLBACK ArxRleRichEditCtrl::StreamInCByteArray(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
...
DWORD CALLBACK ArxRleRichEditCtrl::StreamInCFile(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
...
DWORD CALLBACK ArxRleRichEditCtrl::StreamOutCString(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG* pcb)
{ SCookieString *pCookie = (SCookieString *)dwCookie;
...
DWORD CALLBACK ArxRleRichEditCtrl::StreamOutCByteArray(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG* pcb)
{ SCookieByteArray *pCookie = (SCookieByteArray *)dwCookie;
  long lWriteSize = cb; // We can only Write such much
  *pcb = 0;             // Till now we have nothing written so far
...
DWORD CALLBACK ArxRleRichEditCtrl::StreamOutCFile(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{ SCookieFile *pCookie = (SCookieFile *)dwCookie;
  *pcb = 0;             // Till now we have nothing written so far
  if( pCookie->lStart < 0 )
	{ return (0); // Nothing to do
