...
#include "ArxRleAppEditorReactor.h"
...
ACRX_NO_CONS_DEFINE_MEMBERS(ArxRleAppEditorReactor, AcEditorReactor);

/////////////////////////////////////////////////////////////////////////////
//////////// STATIC CONSTRUCTOR/DESTRUCTOR FOR THIS SINGLETON CLASS /////////
/////////////////////////////////////////////////////////////////////////////

ArxRleAppEditorReactor* ArxRleAppEditorReactor::m_instance = NULL;
...
ArxRleAppEditorReactor::getInstance()
{
    ...
    m_instance = new ArxRleAppEditorReactor;
...
/****************************************************************************
**
**  ArxRleAppEditorReactor::ArxRleAppEditorReactor
**
**  **jma
**
*************************************/
...
/****************************************************************************
**
**  ArxRleAppEditorReactor::databaseConstructed
**
**  **jma
**
*************************************/
...
void
ArxRleAppEditorReactor::databaseConstructed(AcDbDatabase* db)
{
    ArxRleUiTdcSysReactors::databaseConstructed(db);
...
/****************************************************************************
**
**  ArxRleAppEditorReactor::databaseToBeDestroyed
**
**  **jma
**
*************************************/
...
void
ArxRleAppEditorReactor::databaseToBeDestroyed(AcDbDatabase* db)
{
    ArxRleUiTdcSysReactors::databaseDestroyed(db);
...
/****************************************************************************
**
**  ArxRleAppEditorReactor::endDeepClone
**
**  **jma
**
*************************************/
...
void
ArxRleAppEditorReactor::endDeepClone(AcDbIdMapping& idMap)
{
...
/****************************************************************************
**
**  ArxRleAppEditorReactor::verifyClonedReferences
...
void
ArxRleAppEditorReactor::verifyClonedReferences(AcDbIdMapping& idMap)
{
    ArxRleDbEntity* ent;
    AcDbObject* obj;
    Acad::ErrorStatus es;
...
/****************************************************************************
**
**	ArxRleAppEditorReactor::beginDeepCloneXlation
**
**  **jma
**
*************************************/
                            ...
                            AcDbIdMapping& idMap, Acad::ErrorStatus* es)
{
    ...
    AcDbObject* objToClone;
		...
		// This happens from the class ArxRleUiTdcWblockClone where you are allowed
    ...
    if (idMap.deepCloneContext() == AcDb::kDcWblock) {
        ...
        for (int i=0; i<len; i++) {
            ...
            if (acdbOpenAcDbObject(objToClone, handCloneObjects[i], AcDb::kForRead) == Acad::eOk) {
                ...
                if (clonedObj != NULL) {
                    acutPrintf(_T("\nArxRleAppEditorReactor: cloned additional object [%s, %s]"),
                                ArxRleUtils::objToClassStr(clonedObj),
                                ArxRleUtils::objToHandleStr(objToClone, str));
                    clonedObj->close();
                }
                objToClone->close();
            }
		...
		// the cloned dictionary records automatically.
    else if ((ArxRleOptions::m_instance.m_doDictRecordInsertByHand) &&
            ...
            // So, look for all ArxRleDictRecords, if its a dict record,
        ...
        for (int i=0; i<len; i++) {
            ...
            if (es == Acad::eOk) {
                acutPrintf(_T("\nArxRleAppEditorReactor: hand inserting [%s]"), ArxRleUtils::objToClassStr(objToClone));

                    // clone the owner dictionary if one of our dictionary records
                if (objToClone->isKindOf(ArxRleDbDictRecord::desc()))
...
/****************************************************************************
**
**	ArxRleAppEditorReactor::collectAllDictRecords
**
**  **jma
**
*************************************/
...
void
ArxRleAppEditorReactor::collectAllDictRecords(AcDbDatabase* db, AcDbObjectIdArray& objIds)
{
    AcDbDictionary* dict;
	Acad::ErrorStatus es;
...
/****************************************************************************
**
**	ArxRleAppEditorReactor::searchOneDictionary
...
void
ArxRleAppEditorReactor::searchOneDictionary(AcDbDictionary* dict, AcDbObjectIdArray& objIds)
{
    ...
    for (; !dictIter->done(); dictIter->next()) {
        if (acdbOpenAcDbObject(obj, dictIter->objectId(), AcDb::kForRead) == Acad::eOk) {
            if (obj->isKindOf(ArxRleDbDictRecord::desc())) {
...
/****************************************************************************
**
**	ArxRleAppEditorReactor::insertCloneOwnerDict
**
**  **jma
**
*************************************/
...
void
ArxRleAppEditorReactor::insertCloneOwnerDict(const AcDbObjectId& dictId,
                            AcDbDatabase* destDb, AcDbIdMapping& idMap)
{
    ...
    CString dictName;
    if (!ArxRleUtils::lookUpDictEntryName(dictId, dictName)) {
        ArxRleUtils::stopAlertBox(_T("ERROR: Couldn't find old dictionary name during Insert!"));
        return;
    }
    ...
    Acad::ErrorStatus es;
    AcDbDictionary* destDict = ArxRleUtils::openDictionaryForRead(dictName, destDb);
    if (destDict) {
        ...
        if (es == Acad::eOk) {
            ...
            if (pClone != NULL) {
                ...
                if (es != Acad::eOk) {
                    ArxRleUtils::stopAlertBox(_T("ERROR: Could not clone destination dictionary!"));
...
/****************************************************************************
**
**	ArxRleAppEditorReactor::insertCloneMergeDicts
...
void
ArxRleAppEditorReactor::insertCloneMergeDicts(AcDbDictionary* srcDict,
