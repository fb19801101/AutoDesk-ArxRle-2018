...
#include "ArxRleEdMapReactor.h"
#include "AcadUtils\ArxRleUtils.h"


     
ArxRleEdMapReactor* ArxRleEdMapReactor::m_instance = NULL;
...
ArxRleEdMapReactor::getInstance()
{
    ...
    m_instance = new ArxRleEdMapReactor;
...
/****************************************************************************
**
**  ArxRleEdMapReactor::ArxRleEdMapReactor
**
**  **000
**
*************************************/
...
ArxRleEdMapReactor::reset()
{
	...
	ArxRleObjIdMapNode* tmpNode;
	int len = m_objIdMap.length();
	for(int i=0; i<len; i++) {
		tmpNode = static_cast<ArxRleObjIdMapNode*>(m_objIdMap[i]);
		delete tmpNode;
	}
...
/****************************************************************************
**
**  ArxRleEdMapReactor::findObjIdMapNode
...
ArxRleObjIdMapNode*
ArxRleEdMapReactor::findObjIdMapNode(const AcDbObjectId& objId, ArxRleObjIdMapNode::IndexType type)
{
	ArxRleObjIdMapNode* tmpNode;
	int len = m_objIdMap.length();
	for(int i=0; i<len; i++) {
		...
		if((type == ArxRleObjIdMapNode::kOrigExt) && (objId == tmpNode->m_origExtId))
			return tmpNode;

		if((type == ArxRleObjIdMapNode::kTmpImport) && (objId == tmpNode->m_tmpImportId))
			return tmpNode;

		if((type == ArxRleObjIdMapNode::kNewCur) && (objId == tmpNode->m_newCurId))
			return tmpNode;

		if((type == ArxRleObjIdMapNode::kTmpExport) && (objId == tmpNode->m_tmpExportId))
			return tmpNode;

		if((type == ArxRleObjIdMapNode::kNewExt) && (objId == tmpNode->m_newExtId))
			return tmpNode;
	}
...
void
ArxRleEdMapReactor::printObj(LPCTSTR event, const AcDbObjectId& objId) const
{
    CString str;
    AcDbObject* obj;
...
void
ArxRleEdMapReactor::printObj(LPCTSTR event, const AcDbObject* obj) const
{
    CString str;
    acutPrintf(_T("\n%-15s : [%-18s: %s, %s] "), _T("[MAP REACTOR]"), event,
                ArxRleUtils::objToClassStr(obj), ArxRleUtils::objToHandleStr(obj, str));
...
/****************************************************************************
**
**  ArxRleEdMapReactor::endDeepClone
...
void
ArxRleEdMapReactor::endDeepClone(AcDbIdMapping& idMap)
{
	...
	if(m_cloneState == kImportWblock) {
        ...
        for(iter.start(); !iter.done(); iter.next()) {
            if(iter.getMap(idPair) && idPair.isCloned()) {
				tmpNode = findObjIdMapNode(idPair.key(), ArxRleObjIdMapNode::kOrigExt);
				if(tmpNode)
        ...
        for(iter.start(); !iter.done(); iter.next()) {
            if(iter.getMap(idPair) && idPair.isCloned()) {
				tmpNode = findObjIdMapNode(idPair.key(), ArxRleObjIdMapNode::kTmpImport);
				if(tmpNode)
        ...
        for(iter.start(); !iter.done(); iter.next()) {
            if(iter.getMap(idPair) && idPair.isCloned()) {
				tmpNode = findObjIdMapNode(idPair.key(), ArxRleObjIdMapNode::kNewCur);
				if(tmpNode)
        ...
        for(iter.start(); !iter.done(); iter.next()) {
            if(iter.getMap(idPair) && idPair.isCloned()) {
                tmpNode = findObjIdMapNode(idPair.key(), ArxRleObjIdMapNode::kTmpExport);
                if(tmpNode)
...
void         
ArxRleEdMapReactor::endInsert(AcDbDatabase* pTo) 
{
    ...
    for(i=0 ; i<len; i++) {
        ...
        if(es == Acad::eOk) {
            ...
            if(es != Acad::eOk) {
                acutPrintf(_T("\nERROR: could not swap object: (%s)"), ArxRleUtils::rxErrorStr(es));
            }
            else {
                ...
                AcDbObjectIdArray& blkEnts, bool skipErased)
{
    ...
    es = ArxRleUtils::openBlockDef(ACDB_MODEL_SPACE, blkRec, AcDb::kForRead, db);
    if(es != Acad::eOk) {
        ASSERT(0);
...
mapTestImportDwg()
{
    ...
    ArxRleEdMapReactor* mapReactor = ArxRleEdMapReactor::getInstance();
    ...
    Acad::ErrorStatus es;
    es = ArxRleUtils::getFileNameForRead(_T("Drawing To Import"), NULL, _T("dwg"), mapReactor->m_dwgName, FALSE, FALSE);
    if(es != Acad::eOk)
        return;
    ...
    if(es != Acad::eOk) {
        ...
        acutPrintf(_T("\nCan't read file: %s"), ArxRleUtils::rxErrorStr(es));
        return;
    }
    ...
    for(int i=0; i<len; i++) {
        tmpNode = new ArxRleObjIdMapNode;
        tmpNode->m_origExtId = msEntList[i];
        mapReactor->m_objIdMap.append(tmpNode);
    }

    mapReactor->m_cloneState = ArxRleEdMapReactor::kImportWblock;
    es = mapReactor->m_extDatabase->wblock(pTempDb, msEntList, AcGePoint3d::kOrigin);
	mapReactor->m_cloneState = ArxRleEdMapReactor::kNull;
	if(es != Acad::eOk) {
		mapReactor->reset();	// free up and null out db pointer
        acutPrintf(_T("\nwblock process failed: %s"), ArxRleUtils::rxErrorStr(es));
    }
	else {
		mapReactor->m_cloneState = ArxRleEdMapReactor::kImportInsert;
		es = acdbHostApplicationServices()->workingDatabase()->insert(idMatrix, pTempDb);
		mapReactor->m_cloneState = ArxRleEdMapReactor::kNull;
		if(es != Acad::eOk) {
			mapReactor->reset();	// free up and null out db pointer
            acutPrintf(_T("\ninsert process failed: %s"), ArxRleUtils::rxErrorStr(es));
		}
		else
...
mapTestExportDwg()
{
	...
	mapReactor->m_cloneState = ArxRleEdMapReactor::kExportWblock;
	...
	if(es != Acad::eOk) {
		...
		mapReactor->m_cloneState = ArxRleEdMapReactor::kExportInsert;
