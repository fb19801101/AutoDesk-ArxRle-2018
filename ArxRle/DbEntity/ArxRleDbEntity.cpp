...
Adesk::Int16     ArxRleDbEntity::m_version = 0;     // MDI safe

ACRX_DXF_DEFINE_MEMBERS(ArxRleDbEntity, AcDbEntity,
						AcDb::kDHL_1015, AcDb::kMRelease0,
						0, ARXRLE_ENTITY, ArxRle)

/****************************************************************************
**
**  ArxRleDbEntity::ArxRleDbEntity
**
**  **jma
**
*************************************/
...
Acad::ErrorStatus
ArxRleDbEntity::setLocation(const AcGePoint3d& pt)
{
    if (pt != m_origin) {
        assertWriteEnabled();
        m_origin = pt;
...
double
ArxRleDbEntity::rotation() const
{
    assertReadEnabled();

    AcGeVector3d xAxis;
    ArxRleUtils::getEcsXAxis(m_zDir, xAxis);        // get AutoCAD's arbitrary X-Axis
    return xAxis.angleTo(m_xDir, m_zDir);
}

/****************************************************************************
**
**  ArxRleDbEntity::setRotation
**
**  **jma
**
*************************************/
...
Acad::ErrorStatus
ArxRleDbEntity::setRotation(double rot)
{
    assertWriteEnabled();
...
Acad::ErrorStatus
ArxRleDbEntity::setNormal(const AcGeVector3d& nvec)
{
    ...
    if (nvec != m_zDir) {
        ...
        ArxRleUtils::getEcsXAxis(nvec, txdir);       // get AutoCAD's arbitrary X-Axis for this normal
        ArxRleUtils::getEcsXAxis(m_zDir, txdiro);    // get AutoCAD's arbitrary X-Axis for this normal
...
Acad::ErrorStatus
ArxRleDbEntity::dwgInFields(AcDbDwgFiler* filer)
{
	if (ArxRleOptions::m_instance.m_showDwgFilerMessages) {
		CString str, str2;
		acutPrintf(_T("\n%s <DwgIn  : %s>"), ArxRleUtils::objToClassAndHandleStr(this, str),
				ArxRleUtils::filerTypeToStr(filer->filerType(), str2));
	}
...
Acad::ErrorStatus
ArxRleDbEntity::dwgOutFields(AcDbDwgFiler* filer) const
{
	if (ArxRleOptions::m_instance.m_showDwgFilerMessages) {
		CString str, str2;
		acutPrintf(_T("\n%s <DwgOut : %s>"),
				ArxRleUtils::objToClassAndHandleStr(const_cast<ArxRleDbEntity*>(this), str),
				ArxRleUtils::filerTypeToStr(filer->filerType(), str2));
	}
...
Acad::ErrorStatus
ArxRleDbEntity::dxfInFields(AcDbDxfFiler* filer)
{
    Acad::ErrorStatus es;
...
Acad::ErrorStatus
ArxRleDbEntity::dxfOutFields(AcDbDxfFiler* filer) const
{
    Acad::ErrorStatus es;
...
/****************************************************************************
**
**  ArxRleDbEntity::getCloneReferences
**
**  **jma
**
*************************************/
...
void
ArxRleDbEntity::getCloneReferences(AcDb::DeepCloneType type,
...
/****************************************************************************
**
**  ArxRleDbEntity::verifyReferences
...
Acad::ErrorStatus
ArxRleDbEntity::verifyReferences(int& fixedErrorCount, bool fullCheck)
{
...
Acad::ErrorStatus
ArxRleDbEntity::subDeepClone(AcDbObject* pOwner,
              ...
              Adesk::Boolean isPrimary) const
{
	...
	if (ArxRleOptions::m_instance.m_showDeepCloneDetails) {
		CString titleStr, tmpStr;
		titleStr.Format(_T("Beginning -- deepClone: %s"),
				ArxRleUtils::objToClassAndHandleStr(const_cast<ArxRleDbEntity*>(this), tmpStr));
		ArxRleUiTdmIdMap dbox(&idMap, acedGetAcadDwgView(), titleStr);
		dbox.DoModal();
	}
...
Acad::ErrorStatus
ArxRleDbEntity::subWblockClone(AcRxObject* pOwner, AcDbObject*& pClone,
                    AcDbIdMapping& idMap, Adesk::Boolean isPrimary) const
{
	if (ArxRleOptions::m_instance.m_showWblockCloneDetails) {
		CString titleStr, tmpStr;
...
Acad::ErrorStatus
ArxRleDbEntity::subTransformBy(const AcGeMatrix3d& xform)
{
    assertWriteEnabled();
...
void
ArxRleDbEntity::getEcs(AcGeMatrix3d& retVal) const
{
...
Acad::ErrorStatus
ArxRleDbEntity::subGetGripPoints(AcGePoint3dArray& gripPoints,
...
/****************************************************************************
**
**  ArxRleDbAdeskLogo::moveGripPointsAt
**
**  **jma
**
*************************************/
...
Acad::ErrorStatus
ArxRleDbEntity::subMoveGripPointsAt(const AcDbIntArray& indices, const AcGeVector3d& offset)
{
...
void
ArxRleDbEntity::subList() const
{
	AcDbEntity::subList();

	CString str;

	printForListCmd(_T("Location"), ArxRleUtils::ptToStr(location(), str));
	printForListCmd(_T("Rotation"), ArxRleUtils::angleToStr(rotation(), str));
	printForListCmd(_T("Normal"),   ArxRleUtils::vectorToStr(normal(), str));
}

/****************************************************************************
**
**  ArxRleDbEntity::printForListCmd
**
**  **jma
**
*************************************/
...
void
ArxRleDbEntity::printForListCmd(LPCTSTR label, LPCTSTR value) const
{
	if (label == NULL)
		acutPrintf(_T("\n%20s"), _T(""));
	else
