...
#ifndef ARXRLEXDATA_H
#define ARXRLEXDATA_H

#include "ArxRleRbList.h"

class ArxRleXdataList;        // forward declaration

/******************************************************************
**
**  CLASS ArxRleAppXdata
**      encapsulates the xdata for a single application.
**  ArxRleAppXdata can have xdata from many different apps, i.e., a list
**  of ArxRleAppXdata objects (ArxRleAppXdataList).
**
**  **jma
**
****************************/
...
public:
        // constructors/destructors
                    ArxRleAppXdata(LPCTSTR appname, AcDbDatabase* db);
    virtual         ~ArxRleAppXdata();
    ...
    bool		detachList(int tag, ArxRleXdataList& xdataList);
    bool		getList(int tag, ArxRleXdataList& xdataList);
    void		setList(int tag, ArxRleXdataList& xdataList);
...
private:
    ...
    bool		isEndList(resbuf* rb);              // is "}"
        ...
        // outlawed functions (for now)
                    ArxRleAppXdata(const ArxRleAppXdata&);
    ArxRleAppXdata&	operator=(const ArxRleAppXdata&);
};


/****************************************************************************
**
**  CLASS ArxRleXdataList:
**
**  **jma
**
*************************************/
...
friend class ArxRleAppXdata;
friend class ArxRleXdataListIterator;

public:
        // constructors/destructor
                ArxRleXdataList();
    virtual     ~ArxRleXdataList();
...
private:
    ...
    void        appendGenericPoint(const AcGePoint3d& value, int dxfType);
        ...
        // outlawed functions (for now)
						ArxRleXdataList(const ArxRleXdataList&);
    ArxRleXdataList&	operator=(const ArxRleXdataList&);
};
...
public:
        // constructor/destructor
                ArxRleXdataListIterator(ArxRleXdataList& xdata);
    virtual     ~ArxRleXdataListIterator();
...
private:
    ...
    bool        setGenericPoint(const AcGePoint3d& value, int dxfType);
        ...
        // outlawed functions (for now)
								ArxRleXdataListIterator(const ArxRleXdataListIterator&);
    ArxRleXdataListIterator&	operator=(const ArxRleXdataListIterator&);
};

/******************************************************************
**
**  CLASS ArxRleAppXdataList
...
friend class ArxRleAppXdataListIterator;

public:
        // constructors/destructors
                    ArxRleAppXdataList(AcDbObject* const obj);    // must have already opened the object
    virtual         ~ArxRleAppXdataList();

    void            append(ArxRleAppXdata* newApp);
    void            prepend(ArxRleAppXdata* newApp);
    bool			contains(LPCTSTR appName) const;
    int             entries() const;
    bool			isEmpty() const;
    ArxRleAppXdata* at(int index);
    void            removeAndDestroy();
    void            removeAndDestroy(LPCTSTR appName);

private:
    AcArray<ArxRleAppXdata*> m_list;
    AcDbObject*     m_obj;
...
/****************************************************************************
**
**  CLASS ArxRleAppXdataListIterator:
**
**  **jma
**
*************************************/
...
public:
        // constructors/destructor
                ArxRleAppXdataListIterator(const ArxRleAppXdataList& list);
    virtual     ~ArxRleAppXdataListIterator();

    virtual void            toFirst();
    virtual void            toLast();
    virtual void            previous();
    virtual void            next();
    virtual ArxRleAppXdata*	item() const;
    virtual bool			done() const;

private:
        // data members
    int		m_pos;
    const ArxRleAppXdataList&  m_xdataList;

        // outlawed functions (for now)
    ArxRleAppXdataListIterator(const ArxRleAppXdataListIterator&);
    ArxRleAppXdataListIterator&    operator=(const ArxRleAppXdataListIterator&);
};
